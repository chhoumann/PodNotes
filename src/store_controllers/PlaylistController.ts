...
export class PlaylistController extends StoreController<{
	[playlistName: string]: Playlist;
}> {
	private plugin: IPodNotes;

	constructor(
		store: Writable<{ [playlistName: string]: Playlist }>,
		plugin: IPodNotes,
	) {
		super(store);
		this.plugin = plugin;
	...
	protected override onChange(value: { [playlistName: string]: Playlist }) {
