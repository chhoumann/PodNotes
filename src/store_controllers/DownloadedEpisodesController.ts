...
export default class DownloadedEpisodesController extends StoreController<{
	[podcastName: string]: DownloadedEpisode[];
}> {
	private plugin: IPodNotes;

	constructor(
		store: Writable<{ [podcastName: string]: DownloadedEpisode[] }>,
		plugin: IPodNotes,
	) {
		super(store);
		this.plugin = plugin;
	...
	protected override onChange(
		value: { [podcastName: string]: DownloadedEpisode[] },
	) {
