...
import { FilePathTemplateEngine, TranscriptTemplateEngine } from "../TemplateEngine";
...
export class TranscriptionService {
    ...
    async transcribeCurrentEpisode(): Promise<void> {
        ...
        const transcriptPath = FilePathTemplateEngine(this.plugin.settings.transcript.path, currentEpisode);
        ...
        try {
            ...
            const downloadPath = await downloadEpisode(currentEpisode, this.plugin.settings.download.path);
    ...
    private createChunkFiles(chunks: ArrayBuffer[], fileName: string, fileExtension: string, mimeType: string): File[] {
        return chunks.map((chunk, index) => 
            new File([chunk], `${fileName}.part${index}.${fileExtension}`, { type: mimeType })
        );
    }
    ...
    private async transcribeChunks(files: File[], updateNotice: (message: string) => void): Promise<string> {
    ...
    private async saveTranscription(episode: Episode, transcription: string): Promise<void> {
